...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Email_CannotScanQR_Logistic');
  ...
  try {
    ...
    await page.waitForTimeout(1000);    
    await page.getByRole('option', { name: 'Pending Collection' }).click();
    await page.waitForTimeout(1000);
    ...
    try {
      ...
      // Step 6c: Find rows with "Pending Collection"
      const pendingRows = await page.locator('tr:has-text("Pending Collection")').all();
      console.log(`📋 Found ${pendingRows.length} quotations with "Pending Collection" status`);

      if (pendingRows.length > 0) {
        // Get the first pending collection row
        ...
        // Click on the first pending collection
    ...
    await page.waitForTimeout(2000);
    await page.getByText('Collect PO here').click();  
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC005B2',
        'Web_Admin_Email_RTS_CannotScanQR_Logistic',
        timestamp,
        testResult,
        screenshotPath
      );
