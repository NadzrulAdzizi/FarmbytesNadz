...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Email_RTS_Financ_Approval_PurchaseOrder');
  ...
  try {
    ...
    await page.waitForTimeout(1000);
    await page.getByRole('combobox').filter({ hasText: 'Finance Status' }).click();
    await page.waitForTimeout(1000);
    await page.getByRole('option', { name: 'Pending Approval for Payment' }).click();
    await page.waitForTimeout(1000);
    await page.getByText('List of FinanceSearch by').click();

    screenshotPath = path.join(screenshotsDir, `step${step++}_finance_bulk_payment_page_loaded.png`);
    await page.screenshot({ path: screenshotPath });

    // Dynamic approach - Find first pending approval for payment (SINGLE METHOD)
    console.log("Step 6: Finding first pending approval for payment dynamically...");
    ...
    try {
      ...
      // Step 6c: Find rows with "Pending Approval for Payment"
      const pendingRows = await page.locator('tr:has-text("Pending Approval for Payment")').all();
      console.log(`📋 Found ${pendingRows.length} quotations with "Pending Approval for Payment" status`);

      if (pendingRows.length > 0) {
        // Get the first pending approval for payment row
        const firstPendingRow = pendingRows[1];
        ...
        const poId = await poIdLink.textContent();
            ...
            // Click on the first pending approval for payment
            await poIdLink.click();
            console.log(`✅ Clicked on pending approval for payment: ${poId}`);
            await page.waitForTimeout(2000);
            screenshotPath = path.join(screenshotsDir, `step${step++}_pending_approval_opened.png`);
            await page.screenshot({ path: screenshotPath }); 
      }    
       else {
        // Simple fallback - use hardcoded search if no pending found
        console.log("⚠️ No pending approval for payment found, using search fallback...");
        ...
        await page.waitForLoadState('networkidle');
        screenshotPath = path.join(screenshotsDir, `step${step++}_search_approval_opened.png`);
        await page.screenshot({ path: screenshotPath });
      }
      
    } catch (error) {
      console.error("❌ Error finding approval for payment:", error.message);
      throw new Error(`Failed to find and open approval for payment: ${error.message}`);
    }
    await page.waitForTimeout(100);
    // Click on Approve for Payment button
    await page.getByRole('button', { name: 'Approve for Payment' }).click();
    await page.getByRole('textbox').click();
    await page.getByRole('textbox').fill(Data.remarksToDriver);
    screenshotPath = path.join(screenshotsDir, `step${step++}_approve_for_payment_remarks.png`);
    await page.screenshot({ path: screenshotPath });
    ...
    await page.waitForTimeout(150);
    screenshotPath = path.join(screenshotsDir, `step${step++}_submit.png`);
    await page.screenshot({ path: screenshotPath });
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC005E',
        'Web_Admin_Email_RTS_Finance_Approval_PurchaseOrder',
        timestamp,
        testResult,
        screenshotPath
      );
