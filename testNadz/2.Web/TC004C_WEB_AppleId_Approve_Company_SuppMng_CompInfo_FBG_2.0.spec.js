...
test('TC004A - Web Admin Approve Company Supplier Management', async () => {
  ...
  const screenshotsDir = path.resolve('./screenshots/web_AppleId_Company_approve_supplier_management_CompanyInfo');
  ...
  try {
    ...
    await page.getByRole('cell', { name: 'Nadzrul Apple' }).click();
    await page.waitForTimeout(1000);
    ...
    await page.getByText('Nadzrul Apple').click();
    await page.waitForTimeout(1000);
    ...
    const writeSuccess = await writeResultToExcel(
      'WEB_AdminLogin',
      'TC004C',
      'Web_Admin_AppleId_Company_Approve_SupplierManagement_CompanyDetails_HappyFlow',
      timestamp,
      testResult,
      screenshotPath
    );
