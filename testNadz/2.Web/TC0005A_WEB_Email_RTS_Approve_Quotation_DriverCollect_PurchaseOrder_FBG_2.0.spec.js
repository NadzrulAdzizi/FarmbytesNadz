...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Email_RTS_Approve_Quote_DriverCollect');
  ...
  try {
    ...
    await page.getByRole('combobox').filter({ hasText: 'Fulfillment Method' }).click();
    await page.waitForTimeout(150);
    await page.getByRole('option', { name: 'Driver Collect' }).click();
    await page.waitForTimeout(150);
    await page.getByText('List of QuotationsSearch by').click();
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC005A',
        'Web_Admin_Email_RTS_Approve_Quote_DriverCollect',
        timestamp,
        testResult,
        screenshotPath
      );
