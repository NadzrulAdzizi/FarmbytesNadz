...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Email_RTS_Approve_Quote_SelfDrop-off');
  ...
  try {
    ...
    await page.waitForTimeout(150);
    await page.getByRole('option', { name: 'Self Drop-off' }).click();
    await page.waitForTimeout(150);
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC005A1',
        'Web_Admin_Email_RTS_Approve_Quote_SelfDrop-off',
        timestamp,
        testResult,
        screenshotPath
      );
