...
import fs from 'fs';
import { generateUniqueTestData } from './testdata.js';
...
test('Playwright_onWeb', async () => {
  ...
  try {
    ...
    try {
      ...
      // Step 6c: Find rows with "Pending Driver Assignment" status (not "Quotation Status")
      const pendingRows = await page.locator('tr:has-text("Pending Driver Assignment")').all();
      console.log(`📋 Found ${pendingRows.length} quotations with "Pending Driver Assignment" status`);
    ...
    // Assign Driver
    console.log("Step 7: Assign Driver");
    await page.getByRole('button', { name: 'Assign Driver' }).click();
    await page.waitForTimeout(1000);
    await page.locator('span').filter({ hasText: 'Driver Name (optional)' }).getByRole('textbox').click();
    await page.locator('span').filter({ hasText: 'Driver Name (optional)' }).getByRole('textbox').fill(testData.driverName);
    await page.locator('span').filter({ hasText: 'Phone Number (optional)' }).getByRole('textbox').click();
    await page.locator('span').filter({ hasText: 'Phone Number (optional)' }).getByRole('textbox').fill(testData.driverPhone);
    ...
    await page.getByRole('combobox').filter({ hasText: 'Select Vehicle Type' }).click();
    await page.getByRole('option', { name: 'Lorry' }).click();
    await page.locator('span').filter({ hasText: 'Vehicle Number' }).getByRole('textbox').click();
    await page.locator('span').filter({ hasText: 'Vehicle Number' }).getByRole('textbox').fill(testData.vehicleNumber);
    await page.locator('div').filter({ hasText: /^Remarks to Driver \(If any\)$/ }).getByRole('textbox').click();
    await page.locator('div').filter({ hasText: /^Remarks to Driver \(If any\)$/ }).getByRole('textbox').fill(testData.remarksToDriver);
    screenshotPath = path.join(screenshotsDir, `step${step++}_remarks_filled.png`);
    await page.screenshot({ path: screenshotPath });

    //Submit Assigned Driver
    console.log("Step 8: Submit Assigned Driver");
    await page.getByRole('button', { name: 'Assign Driver' }).click();
    await page.getByRole('button', { name: 'Driver Out For Collection' }).click();
