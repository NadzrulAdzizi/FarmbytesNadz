...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_AppleId_approve_Individual_supplier_management_BankDetails');
  ...
  try {
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC004B',
        'Web_Admin_AppleId_Approve_Individual_SupplierManagement_BankDetails_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
