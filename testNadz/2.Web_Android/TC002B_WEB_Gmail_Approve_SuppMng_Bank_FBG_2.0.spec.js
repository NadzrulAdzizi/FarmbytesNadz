...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Gmail_approve_supplier_management_BankDetails');
  ...
  try {
    ...
    await page.getByRole('textbox').click();
    await page.getByRole('textbox').fill('Mohd');
    await page.waitForTimeout(1000);
    ...
    await page.getByRole('link', { name: 'Mohd Nadzrul Adzizi' }).click();
    await page.waitForLoadState('networkidle');
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC002B',
        'Web_Admin_Gmail_Approve_SupplierManagement_BankDetails_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
