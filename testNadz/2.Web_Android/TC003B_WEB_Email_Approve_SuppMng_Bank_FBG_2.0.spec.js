...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Email_approve_supplier_management_BankDetails');
  ...
  try {
    ...
    // Step 8: Enter search text for AUTOTEST
    console.log("Step 8: Enter search text for AUTOTEST");
    await page.getByRole('textbox').fill('autotest');
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_search_text_entered.png`);
    await page.screenshot({ path: screenshotPath });
/*
    // Step 9: Set KYC Status filter to Pending
    console.log("Step 9: Set KYC Status filter to Pending");
    await page.getByRole('combobox').filter({ hasText: 'KYC Status' }).click();
    await page.waitForTimeout(500);
    await page.getByRole('option', { name: 'Pending' }).click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_kyc_status_pending_selected.png`);
    await page.screenshot({ path: screenshotPath });
*/
    // Step 10: Set Bank Verification Status filter to Pending
    console.log("Step 10: Set Bank Verification Status filter to Pending");
    await page.getByRole('combobox').filter({ hasText: 'Bank Verification Status' }).click();
    await page.waitForTimeout(500);
    await page.getByRole('option', { name: 'Pending' }).click();
    await page.waitForTimeout(2000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_bank_status_pending_selected.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 11: Find and click on the latest AUTOTEST user
    console.log("Step 11: Find and click on the latest AUTOTEST user");
    try {
      // Wait for the table to load after filtering
      await page.waitForSelector('table tbody tr', { timeout: 10000 });
      
      // Method 1: Try to find the latest AUTOTEST user by finding highest number
      const autotestLinks = await page.locator('a[href*="supplier"]').filter({ hasText: /AUTOTEST\d+/i }).all();
      
      if (autotestLinks.length > 0) {
        console.log(`Found ${autotestLinks.length} AUTOTEST users`);
        
        // Extract numbers from AUTOTEST links and find the highest
        let latestLink = null;
        let highestNumber = 0;
        
        for (const link of autotestLinks) {
          const linkText = await link.textContent();
          const match = linkText.match(/AUTOTEST(\d+)/i);
          if (match) {
            const number = parseInt(match[1]);
            console.log(`Found AUTOTEST${number}`);
        ...
        if (latestLink) {
          const latestLinkText = await latestLink.textContent();
          console.log(`Clicking on latest AUTOTEST user: ${latestLinkText}`);
          await latestLink.click();
        } else {
          console.log("No numbered AUTOTEST users found, clicking first AUTOTEST link");
          await autotestLinks[0].click();
        }
      } else {
        // Fallback: Try to find any AUTOTEST link (case insensitive)
        console.log("No AUTOTEST links found with specific pattern, trying general search");
        const generalAutotestLink = page.locator('a').filter({ hasText: /autotest/i }).first();
        
        if (await generalAutotestLink.isVisible()) {
          const linkText = await generalAutotestLink.textContent();
          console.log(`Clicking on AUTOTEST user: ${linkText}`);
          await generalAutotestLink.click();
        } else {
          // Last resort: click on any pending user
          console.log("No AUTOTEST users found, clicking on first pending user");
          await page.locator('tr:has-text("Pending") a').first().click();
        }
      }
    } catch (findError) {
      console.error("❌ Error finding AUTOTEST user:", findError.message);
      // Emergency fallback
      try {
        await page.locator('tr:has-text("Pending") a').first().click();
        console.log("⚠️ Used emergency fallback - clicked first pending user");
      } catch (emergencyError) {
        throw new Error("Unable to find any user to approve");
      }
    }
    
    await page.waitForLoadState('networkidle');
    screenshotPath = path.join(screenshotsDir, `step${step++}_latest_autotest_user_selected.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 21: Navigate to Farm Information tab
    console.log("Step 21: Navigate to Farm Information tab");
    await page.getByRole('tab', { name: 'Farm Information' }).click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_farm_information_tab.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 22: Navigate to List of Employees tab
    console.log("Step 22: Navigate to List of Employees tab");
    await page.getByRole('tab', { name: 'List of Employees' }).click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_employees_tab.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 23: Navigate to Bank Details tab
    console.log("Step 23: Navigate to Bank Details tab");
    await page.getByRole('tab', { name: 'Bank Details' }).click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_bank_details_tab.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 24: Verify bank details
    console.log("Step 24: Verify bank details");
    await page.getByRole('combobox').click();
    await page.waitForTimeout(3000);
    await page.getByRole('option', { name: 'Verified' }).click();
    await page.waitForTimeout(4000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_bank_details_verified.png`);
    await page.screenshot({ path: screenshotPath });
