...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_Gmail_approve_supplier_management_CompanyInfo');
  ...
  try {
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC002A',
        'Web_Admin_Gmail_Approve_SupplierManagement_CompanyDetails_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
