...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_FB_approve_supplier_management_CompanyInfo');
  ...
  try {
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC001A',
        'Web_Admin_FB_Approve_SupplierManagement_CompanyInfo_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC001A',
        'Web_Admin_FB_Approve_SupplierManagement_CompanyDetails_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
