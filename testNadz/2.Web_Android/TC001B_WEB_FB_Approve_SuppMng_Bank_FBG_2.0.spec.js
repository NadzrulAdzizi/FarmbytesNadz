...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_FB_approve_supplier_management_BankDetails');
  ...
  try {
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC001B',
        'Web_Admin_FB_Approve_SupplierManagement_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
    ...
    try {
      const success = await writeResultToExcel(
        'WEB_AdminLogin',
        'TC001B',
        'Web_Admin_FB_Approve_SupplierManagement_BankDetails_HappyFlow',
        timestamp,
        testResult,
        screenshotPath
      );
