...
test('Playwright_onWeb', async () => {
  let testResult = 'PASS';
  let screenshotPath = '';
  const screenshotsDir = path.resolve('./screenshots/web_login_approve_supplier_management');
  ...
  try {
    ...
    await page.getByRole('textbox').click();
    await page.getByRole('textbox').fill('fbc');
    await page.waitForTimeout(1000);
    ...
    await page.getByRole('link', { name: 'Fbc Autojr' }).click();
    await page.waitForLoadState('networkidle');
    ...
    // Step 25: Click Logout button
    console.log("Step 25: Click Logout button");
    await page.getByRole('button', { name: 'Logout' }).click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_logout_clicked.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 26: Confirm logout
    console.log("Step 26: Confirm logout");
    await page.getByRole('button', { name: 'Logout' }).first().click();
    await page.waitForTimeout(1000);
    screenshotPath = path.join(screenshotsDir, `step${step++}_logout_confirmed.png`);
    await page.screenshot({ path: screenshotPath });

    // Step 27: Return to login page
    console.log("Step 27: Return to login page");
    await page.getByRole('link', { name: 'Login' }).click();
    await page.waitForLoadState('networkidle');
    screenshotPath = path.join(screenshotsDir, `step${step++}_back_to_login.png`);
    await page.screenshot({ path: screenshotPath });
    ...
    // Write results to Excel with multiple fallback methods
    console.log("📊 Writing results to file...");
    console.log(`📋 Final data: testResult=${testResult}, screenshotPath=${screenshotPath}`);
    ...
    if (success) {
      ...
      console.error("❌ All file writing methods failed!");
    }
