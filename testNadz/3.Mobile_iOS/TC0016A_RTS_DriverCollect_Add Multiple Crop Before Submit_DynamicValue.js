...
async function main() {
  ...
  const screenshotDir = path.resolve('./screenshots/Mobile_iOS_TC0016A_DriverCollect_AddMultipleCropBeforeSubmit');
  ...
  try {
    ...
    // Add Multiple Crops - Dynamic Value
    console.log("Adding Multiple Crops - Dynamic Value");
    await clickWithRetries(driver, '(//XCUIElementTypeOther[@name="TOMATO RM 8.00 /kg"])[2]', 'Add Item Button');
    await driver.pause(1000);
    screenshotPath = path.join(screenshotDir, `step${step++}_item_added.png`);
    await driver.saveScreenshot(screenshotPath);

    // AUTOMATED Fill value with random amount
    console.log("Auto-Fill Random Value");
    
    const filledValue3 = await fillRandomValue(
      driver, 
      '//XCUIElementTypeTextField[@value="Enter Value"]', 
      150,    // minimum value
      300,    // maximum value
    ...
    if (filledValue3) {
      console.log(`🎯 Successfully filled value: ${filledValue3} kg`);
    } else {
      console.log("⚠️ Auto-fill failed, using fallback");
      await fillTextWithRetries(driver, '//XCUIElementTypeTextField[@value="Enter Value"]', "200", 'Value Field');
    }
    ...
    await driver.pause(1000);
    await clickWithRetries(driver, '~Done', 'Done button');
    screenshotPath = path.join(screenshotDir, `step${step++}_value_filled_${filledValue3 || 'fallback'}.png`);
    await driver.saveScreenshot(screenshotPath);
    
    // FIXED: Swipe up to reveal more options
    console.log("Step 6.5: Swipe up to reveal Add Item button");
    const swipeSuccess3 = await performSwipe(driver, 'up', 3);
    if (!swipeSuccess3) {
      console.log("⚠️ Swipe failed, but continuing with test");
    }
    await driver.pause(1000);
    ...
    await driver.pause(1000);
    screenshotPath = path.join(screenshotDir, `step${step++}_after_swipe_up.png`);
    await driver.saveScreenshot(screenshotPath);
    ...
    await clickWithRetries(driver, '//XCUIElementTypeOther[@name="   Confirm Request  "]');
    await driver.pause(2000);
    ...
    const writeSuccess = await writeResultToExcel(
      ...
      'TC0016A',
      'iOS_DriverCollect_AddMultipleCropBeforeSubmit_HappyFlow',
      testResult,
      screenshotPath,
      'Mobile_iOS'
    );
